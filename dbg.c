...
static uint32_t fetch_instr(const cpuj_t *cpu, uint32_t addr, int *len) {
    *len = 4;
    const uint8_t *r = cpu->ram;
    return ((uint32_t)r[addr]     << 24) |
           ((uint32_t)r[addr + 1] << 16) |
           ((uint32_t)r[addr + 2] << 8)  |
           ((uint32_t)r[addr + 3]);
}
...
void dbg_print_regs(const cpuj_t *cpu) {
    for (int i = 0; i < CPUJ_REG_COUNT; i++) {
        printf("R%d=%08X (%10u)  %s", i, cpu->r[i], cpu->r[i],
               (i % 2) ? "\n" : "");
    }
    printf("SP=%08X  PC=%08X  FLAGS=%s%s%s%s%s",
           cpu->sp, cpu->pc,
...
void dbg_print_mem(const cpuj_t *cpu, uint32_t from, uint32_t to) {
    for (uint32_t a = from; a <= to; a += 16) {
        printf("%08X:", a);
        for (int i = 0; i < 16 && (uint64_t)a + (uint32_t)i <= to; i++)
            printf(" %02X", cpu->ram[(uint32_t)(a + (uint32_t)i)]);
        printf("\n");
    }
}
...
void dbg_print_insn(const cpuj_t *cpu, uint32_t addr) {
    ...
    printf("  %08X: ", addr);
    for (int i = 0; i < len; i++)
        printf("%02X", cpu->ram[(uint32_t)(addr + (uint32_t)i)]);
    printf("  %s\n", desc);
}
...
static bool at_breakpoint(const dbg_t *dbg, uint32_t pc) {
    for (int i = 0; i < dbg->nbreak; i++)
    ...
    for (;;) {
        ...
        if (strcmp(cmd, "h") == 0 || strcmp(cmd, "help") == 0) {
            printf("  s [n]      step n instructions (default 1)\n"
                   ...
                   "  m [lo] [hi] dump memory (default 0x00000000..0x0000000F)\n"
            ...
            char *hi = strtok(NULL, " \t");
            uint32_t a = 0, b = 0x0F;
            if (lo) { uint32_t tmp; if (parse_addr(lo, &tmp) == 0) a = tmp; }
            if (hi) { uint32_t tmp; if (parse_addr(hi, &tmp) == 0) b = tmp; }
            if (b < a) { uint32_t t = a; a = b; b = t; }
            dbg_print_mem(cpu, a, b);
        } else if (strcmp(cmd, "d") == 0 || strcmp(cmd, "disasm") == 0) {
            char *arg = strtok(NULL, " \t");
            uint32_t a = cpu->pc;
            if (arg) { uint32_t tmp; if (parse_addr(arg, &tmp) == 0) a = tmp; }
            for (int i = 0; i < 5; i++) {
